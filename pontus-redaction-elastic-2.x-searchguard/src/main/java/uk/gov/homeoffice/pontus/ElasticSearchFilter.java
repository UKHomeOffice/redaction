...
//                LOG.trace(String.format("Header %s", request.getHeaders()));
      ...
      if (useSearchGuard) {
        ...
        User elasticSearchGuardUser = request.getFromContext(ConfigConstants.SG_USER);
        if (elasticSearchGuardUser == null && request.remoteAddress() == null) {
          elasticSearchGuardUser = ElasticSearchGuardUser.SG_INTERNAL;
        }
        if (elasticSearchGuardUser != null) {
          userStr = elasticSearchGuardUser.getName();
        }
        else {
          userStr = "_sg_internal";
        }
      }
      else if (useShield) {

        userStr = request.getFromContext("shield.user"); // request.getFromContext("shield.user");
...
//                    }
    ...
    try {
      ...
      if (request instanceof SearchRequest) {
        ...
        String redactionElasticPostFilterQueryStr = filterData.getRedactionElasticPostFilterQueryStr();
        ...
        if (redactionElasticPostFilterQueryStr == null) {
          ...
          if (FilterData.RedactionType.FILTER == filterData.getRedactionType()) {
            newQb.must(new RegexpQueryBuilder("_all", filterData.getRedactionAllowedStr()))
              .mustNot(new RegexpQueryBuilder("_all", filterData.getRedactionDeniedStr()))
              .mustNot(new RegexpQueryBuilder("_all", filterData.getRedactionDeniedAllStr()));
          }
        ...
        if (!skipRedaction) {
          ...
          JSONObject obj = source == null ? new JSONObject() : new JSONObject(source.toUtf8());
          ...
          postFilter = new JSONObject(newQb.toString());
          obj.put("post_filter", postFilter);

          searchReq.source(obj.toString());
        }

        chain.proceed(task, action, searchReq, listener);
