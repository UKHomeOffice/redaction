...
public class SolrPoleSecurityJSONWriterFilter extends JSONWriter {
    protected static final Log LOG = LogFactory.getLog(SolrPoleSecurityJSONWriterFilter.class);
    ...
    public SolrPoleSecurityJSONWriterFilter(Writer writer, SolrQueryRequest req, SolrQueryResponse rsp) throws KeeperException, InterruptedException, IOException {
        super(writer,req,rsp);

        Principal princ = req.getUserPrincipal();
        LOG.info(String.format("Creating new JSON writer filter for principal %s", princ));

        JWTUser user = new JWTUser(princ);

        LOG.info(String.format("Creating new JSON writer filter for user %s", user));
        claim = jwtStore.getUserClaim(user);

        LOG.info(String.format("Creating new JSON writer filter for claim %s", claim));
