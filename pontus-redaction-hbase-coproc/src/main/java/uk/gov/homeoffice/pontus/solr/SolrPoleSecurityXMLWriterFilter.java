 ...
 */
public class SolrPoleSecurityXMLWriterFilter extends XMLWriter {
    protected static final Log LOG = LogFactory.getLog(SolrPoleSecurityXMLWriterFilter.class);
    ...
    public SolrPoleSecurityXMLWriterFilter(Writer writer, SolrQueryRequest req, SolrQueryResponse rsp) throws KeeperException, InterruptedException, IOException {
        ...
        LOG.info(String.format("Creating new XML writer filter for user %s", user));
        claim = jwtStore.getUserClaim(user);

        LOG.info(String.format("Creating new XML writer filter for claim %s", claim));
