...
/**
 * Created by leo on 16/11/2016.
 */
public class SolrPontusRequestHandler extends RequestHandlerBase implements SolrCoreAware, PluginInfoInitialized
{
    static final String INIT_COMPONENTS = "components";
    static final String INIT_FIRST_COMPONENTS = "first-components";
    static final String INIT_LAST_COMPONENTS = "last-components";

    private static final Logger log = LoggerFactory.getLogger(SolrPontusRequestHandler.class);
    ...
    public void handleRequestBody(SolrQueryRequest req, SolrQueryResponse rsp) throws Exception
    {
        ...
        SolrParams fqParams = req.getParams();
        if (fqParams instanceof MultiMapSolrParams){
            Map<String, String[]> map = ((MultiMapSolrParams) fqParams).getMap();
            String[] fq = map.get("fq");
            boolean found = false;
            if (fq != null) {

                for (int i = 0, ilen = fq.length; i < ilen; i++) {
                    String filterStr = fq[i];
                    if ("{!PontusSecurity}".equals(filterStr)){
                        found = true;
                        break;
                    }
            ...
            if (!found) {
                if (fq != null) {
                    String[] newFq = new String[fq.length + 1];
                    System.arraycopy(fq, 0, newFq, 0, fq.length);
                    newFq[fq.length] = "{!PontusSecurity}";
                    map.put("fq", newFq);
                }
                else{
                    String[] newFq = new String[]{"{!PontusSecurity}"};
                    map.put("fq", newFq);
                }
            }
                ...
                while (rb.outgoing.size() > 0) {
                    ...
                    while (rb.outgoing.size() > 0) {
                        ...
                        for (String shard : sreq.actualShards) {
                            ...
                            if (princ != null){
                                params.set("principal", princ.getName() );
