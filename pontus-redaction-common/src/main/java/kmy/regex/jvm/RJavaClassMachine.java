...
public class RJavaClassMachine extends RMachine implements CharClassCodes, TokenConst {
    ...
    int V_START = -1; // used only for embedded regex
    int V_BEGIN = -1; // used only for embedded regex
    int V_MAXSTART = -1; // used only for embedded regex
    ...
    public void init() {
        try {
                ...
                if (gen == null) {
                    if (thisClass == null)
                        synchronized (counterLock) {
                            counter++;
                            thisClass = "kmy/regex/jvm/R_tmp" + counter;
                        }
            ...
            if ((flags & HINT_CHAR_STAR_HEAD) != 0) {
                ...
                gen.loadConst(new Integer(1));
                gen.store(V_HEADINC, "I");
            }

            if (!embed) {
                gen.load(0, thisType);
                gen.getfield(stubClass, "string", charArrType);
                gen.store(V_STRING, charArrType);
    ...
    private void maskJumpIfInRange(char[] ranges, int first, int last,
        ...
        gen.load(V_TMP1, charType);
        gen.loadConst(new Integer(fval));
        gen.op('-', "I");
        gen.dup("I");
        int V_TMP2 = V_RET1; // we can reuse V_RET1
        gen.store(V_TMP2, "I");
        gen.loadConst(new Integer(0xFFFFFFE0));
        gen.op('&', "I");
        ...
        gen.loadConst(new Integer(mask));
        gen.op('&', "I");
    ...
    private void genRealloc() throws IOException {
        gen.load(V_FORKS, "[I");
        gen.arraylength();
        gen.loadConst(new Integer(2));
        gen.op('*', "I");
        gen.newarray("I");
        gen.dup("[I");
        gen.load(V_FORKS, "[I");
        gen.swap();
        gen.loadConst(new Integer(0));
        gen.swap();
        gen.loadConst(new Integer(0));
        gen.load(V_FORKS, "[I");
        gen.arraylength();
    ...
    public void finish() {
        try {
            ...
            if (refillMark != null) {
                ...
                gen.load(0, thisType);
                gen.loadConst(new Integer(Integer.MAX_VALUE));
                ...
                if ((flags & HINT_START_ANCHORED) == 0) {
                    ...
                    if (minLength > 0)
                        if (V_REFILLER < 0) {
                            gen.loadConst(new Integer(minLength));
                            gen.op('-', "I");
                        } else {
                            gen.load(V_REFILLER, refillerType);
            ...
            gen.loadConst("Internal error during regex matching");
            ...
            while (keys.hasMoreElements()) {
                ...
                Variable[] v = (Variable[]) vars.get(key);
                gen.dup("Ljava/util/Hashtable;");
                gen.loadConst(key);
                gen.loadConst(THREE);
                gen.newarray("I");
                gen.dup("[I");
                gen.loadConst(ZERO);
                gen.loadConst(new Integer(v[0].cell));
                gen.putelement("I");
                gen.dup("[I");
                gen.loadConst(ONE);
                gen.loadConst(new Integer(v[1].cell));
                gen.putelement("I");
                gen.dup("[I");
                gen.loadConst(TWO);
                gen.loadConst(new Integer(v[0].extCell));
                gen.putelement("I");
            ...
            if (varCells != 0) {
                ...
                gen.load(0, thisType);
                gen.loadConst(new Integer(varCells));
                gen.newarray("I");
            ...
            if (extCells != 0) {
                ...
                gen.load(0, thisType);
                gen.loadConst(new Integer(extCells));
                gen.newarray("[C");
                gen.putfield(stubClass, "extCells", "[[C");
            }
            ...
            gen.startMethod(gen.ACC_PUBLIC, "init", "([CII)V", null);

            //--> this.string = arr
            gen.load(0, thisType);
            gen.load(1, "[C");
            gen.putfield(stubClass, "string", "[C");
            ...
            Enumeration varList = vars.elements();
            while (varList.hasMoreElements()) {
                Variable[] pair = (Variable[]) varList.nextElement();
                if (pair[0].extCell >= 0)
                    continue;
                ...
                for (int q = 0; q < 2; q++) {
                    ...
                    gen.load(1, "[I");
                    gen.loadConst(new Integer(pair[q].cell));
            ...
            gen.load(0, thisType);
            if ((flags & HINT_END_ANCHORED) != 0 && maxLength < 2048) {
                gen.load(0, thisType);
                gen.getfield(stubClass, "end", "I");
                gen.loadConst(new Integer(maxLength));
                gen.op('-', "I");
                gen.dup("I");
                gen.load(2, "I");
	...
	*/
            ...
            if (gen instanceof JVMClassGenerator) {
                ...
                if (saveBytecode) {
                    ...
                    File directory = new File(name).getParentFile().getAbsoluteFile();
                    if (!directory.exists()) {
                        directory.mkdirs();
                    }
                ...
                if (loadClass)
                    compiledClass = loader.makeClass(thisClass, body);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public Regex makeRegex() {
        try {
            return (JavaClassRegexStub) compiledClass.newInstance();
    ...
    public RVariable newTmpVar(int init) {
        ...
        v.cell = varCells++;
        try {
            gen.loadConst(new Integer(v.cell));
            if (saveMark == null)
                saveMark = gen.newMark();
            gen.jsr(saveMark, -1);
            gen.load(V_CELLS, "[I");
            gen.loadConst(new Integer(v.cell));
            gen.loadConst(new Integer(init));
            gen.putelement("I");
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void hardAssign(RVariable v, int value) {
        try {
            gen.load(V_CELLS, "[I");
            gen.loadConst(new Integer(((Variable) v).cell));
            gen.loadConst(new Integer(value));
            gen.putelement("I");
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void fork(RLabel forkLabel) {
        try {
            ...
            gen.load(V_FORKS, "[I");
            gen.load(V_FORKPTR, "I");
            gen.loadConst(new Integer(-index));
            gen.putelement("I");
    ...
    public void skip() {
        try {
            ...
            if ((flags & FLAG_DOT_IS_ANY) == 0) {
                gen.load(V_STRING, charArrType);
                gen.load(V_HEAD, "I");
                gen.getelement(charType);
                gen.loadConst(new Integer('\n'));
    ...
    public void assertFn(int charClass, char[] ranges) {
        try {
            AbstractMark againMark = gen.newMark();
            gen.mark(againMark);
            gen.load(V_HEAD, "I");
    ...
    public void assertFn(char[] constStr) {
        try {
            AbstractMark againMark = gen.newMark();
            gen.mark(againMark);
            gen.load(V_HEAD, "I");
            if (constStr.length == 1) {
                ...
                gen.loadConst(new Integer(constStr.length - 1));
                gen.op('+', "I");
                gen.load(V_END, "I");
            }
            refillIfHaveTo(againMark);
            for (int i = 0; i < constStr.length; i++) {
                ...
                gen.load(V_HEAD, "I");
                gen.getelement(charType);
                gen.loadConst(new Integer(constStr[i]));
                gen.jumpIf(false, gen.TOKEN_NE, "I", failMark);
                gen.iinc(V_HEAD, 1);
            }
        } catch (IOException e) {
    ...
    public void assertFn(String varName, boolean picked) {
        try {
            Variable[] v = (Variable[]) vars.get(varName);
            if (v == null)
                if (picked)
            ...
            if (v[0].extCell >= 0) {
                ...
                if (embed) {
                    ...
                    gen.load(0, thisType);
                    gen.getfield(stubClass, "extCells", charArrArrType);
                    gen.loadConst(new Integer(v[0].extCell));
                    gen.getelement(charArrType);
    ...
    public void decjump(RVariable var, RLabel label) {
        try {
            Integer cell = new Integer(((Variable) var).cell);
            gen.loadConst(cell);
            if (saveMark == null)
                saveMark = gen.newMark();
            gen.jsr(saveMark, -1);
    ...
    public void mfEnd(int maxCount) {
        ...
        try {
            ...
            if (maxCount < Integer.MAX_VALUE) {
                ...
                gen.loadConst(new Integer(maxCount));
            ...
            if ((flags & HINT_CHAR_STAR_HEAD) != 0) {
                ...
                gen.load(V_MFCOUNT, "I");
                gen.loadConst(new Integer(mfMinCount + 1));
                gen.op('+', "I");
                gen.store(V_HEADINC, "I");
            }
            ...
            gen.load(V_MFCOUNT, "I");
            if (mfHeadDecrement != 1) {
                gen.loadConst(new Integer(mfHeadDecrement));
                gen.op('*', "I");
            }
            gen.load(V_MFHEAD, "I");
            gen.op('+', "I");
    ...
    public void condJump(int atLeast, int atMost, RLabel label) {
        ...
        try {
            if (atLeast > 0) {
                gen.load(V_HEAD, "I");
                gen.loadConst(new Integer(atLeast));
                gen.op('+', "I");
                gen.load(V_END, "I");
	...
	// in case of embedded regex, when we know for sure, as checking
	...
	*/
    ...
    public void shiftTable(boolean beginning, int charsAhead, char[] chars, int[] shifts) {
        ...
        try {
            ...
            gen.load(V_HEAD, "I");
            gen.loadConst(new Integer(charsAhead));
            gen.op('+', "I");
	...
	*/
            ...
            gen.load(V_STRING, charArrType);
            gen.load(V_TMP1, "I");
            gen.getelement(charType); // stack now has: lookup lookupChar
            ...
            if (2 * tableSize < chars.length + 5) {
                ...
                for (int i = 0; i < tableSize; i++) {
                    if (chars[j] == caseValue) {
                        if (shifts[j] != 0) {
                            ...
                            gen.loadConst(new Integer(shifts[j]));
                            gen.op('+', "I");
                            gen.jump(tryAgain);
                        }
                        j++;
                ...
                for (int i = 0; i < tableSize; i++) {
                    if (shifts[i] != 0) {
                        gen.mark(markTable[i]);
                        gen.loadConst(new Integer(shifts[i]));
                        gen.op('+', "I");
                        gen.jump(tryAgain);
                    }
                }
