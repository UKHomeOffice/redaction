...
public class LruHashMap<K extends HeapSize, V extends HeapSize>
    ...
    private static final int OVERHEAD = 5 * 8 /*(Bytes.SIZEOF_LONG)*/ +
            2 * 4 /* Bytes.SIZEOF_INT*/ + 2 * 4 /* Bytes.SIZEOF_FLOAT */ + 3 * ClassSize.REFERENCE +
            1 * ClassSize.ARRAY;
    ...
    private LruHashMap.Entry<K,V> headPtr;
    /** Pointer to most recently used entry */
    private LruHashMap.Entry<K,V> tailPtr;
    ...
    public LruHashMap(int initialCapacity, float loadFactor,
        ...
        this.entries = new LruHashMap.Entry[capacity];
    ...
    public synchronized V get(Object key) {
        ...
        LruHashMap.Entry<K,V> e = entries[i];
        while (true) {
            if (e == null) {
                missCount++;
                return null;
    ...
    public synchronized V put(K key, V value) {
        ...
        // For old values
        for (LruHashMap.Entry<K,V> e = entries[i]; e != null; e = e.next) {
            if (e.hash == hash && isEqual(key, e.key)) {
                V oldValue = e.value;
    ...
    public synchronized V remove(Object key) {
        LruHashMap.Entry<K,V> e = removeEntryForKey((K)key);
    ...
    public synchronized boolean containsValue(Object value) {
        checkValue((V)value);
        LruHashMap.Entry[] tab = entries;
        for (int i = 0; i < tab.length ; i++)
            for (LruHashMap.Entry e = tab[i]; e != null ; e = e.next)
    ...
    private void updateLru(LruHashMap.Entry<K,V> e) {
        LruHashMap.Entry<K,V> prev = e.getPrevPtr();
        LruHashMap.Entry<K,V> next = e.getNextPtr();
        if(next != null) {
            if(prev != null) {
    ...
    private void removeEntry(LruHashMap.Entry<K,V> entry) {
        ...
        int hash = entry.hash;
        int i = hashIndex(hash, entries.length);
        LruHashMap.Entry<K,V> prev = entries[i];
        LruHashMap.Entry<K,V> e = prev;

        while (e != null) {
            LruHashMap.Entry<K,V> next = e.next;
            if (e.hash == hash && isEqual(k, e.key)) {
                ...
                LruHashMap.Entry<K,V> prevPtr = e.getPrevPtr();
                LruHashMap.Entry<K,V> nextPtr = e.getNextPtr();
    ...
    private long addEntry(int hash, K key, V value, int bucketIndex) {
        LruHashMap.Entry<K,V> e = entries[bucketIndex];
        LruHashMap.Entry<K,V> newE = new LruHashMap.Entry<K,V>(hash, key, value, e, tailPtr);
    ...
    private void growTable(int newCapacity) {
        LruHashMap.Entry[] oldTable = entries;
        ...
        LruHashMap.Entry[] newTable = new LruHashMap.Entry[newCapacity];

        // Transfer existing entries to new hash table
        for(int i=0; i < oldCapacity; i++) {
            LruHashMap.Entry<K,V> entry = oldTable[i];
            if(entry != null) {
                // Set to null for GC
                oldTable[i] = null;
                do {
                    LruHashMap.Entry<K,V> next = entry.next;
    ...
    public List<LruHashMap.Entry<K,V>> entryLruList() {
        List<LruHashMap.Entry<K,V>> entryList = new ArrayList<LruHashMap.Entry<K,V>>();
        LruHashMap.Entry<K,V> entry = headPtr;
        while(entry != null) {
            entryList.add(entry);
    ...
    public Set<LruHashMap.Entry<K,V>> entryTableSet() {
        Set<LruHashMap.Entry<K,V>> entrySet = new HashSet<LruHashMap.Entry<K,V>>();
        LruHashMap.Entry[] table = entries; // FindBugs IS2_INCONSISTENT_SYNC
        for(int i=0;i<table.length;i++) {
            for(LruHashMap.Entry e = table[i]; e != null; e = e.next) {
                entrySet.add(e);
            }
        }
        return entrySet;
    ...
    protected static class Entry<K extends HeapSize, V extends HeapSize>
        ...
        static final int OVERHEAD = 1 * 8 /*Bytes.SIZEOF_LONG */+
                5 * ClassSize.REFERENCE + 2 * 4/* Bytes.SIZEOF_INT */;
         ...
         */
        Entry(int h, K k, V v, LruHashMap.Entry<K,V> nextChainPtr, LruHashMap.Entry<K,V> prevLruPtr) {
            value = v;
            next = nextChainPtr;
            key = k;
            hash = h;
